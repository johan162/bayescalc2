...
class QueryParser:
    ...
    def _validate_and_normalize_values(self, query_vars_names: List[str], 
                                  evidence: Dict[str, str], 
                                  query_evidence: Dict[str, str]) -> Tuple[Dict[str, str], Dict[str, str]]:
        """
        ...
        for var, val in {**evidence, **query_evidence}.items():
            ...
            if val not in self.network.variables[var].domain:
                ...
                if self.network.variables[var].is_boolean:
                    if (val == "T" and "True" in self.network.variables[var].domain) or \
                       (val == "F" and "False" in self.network.variables[var].domain):
                        continue
        ...
        for item in evidence_part.split(','):
            ...
            if item.startswith('~'):
                ...
                if not self.network.variables[var_name].is_boolean:
                    raise ValueError(f"Negation (~) can only be used with boolean variables, but '{var_name}' is not boolean")
            ...
            if '=' in item:
                ...
                var_name = item.strip()
                if var_name in self.network.variables and self.network.variables[var_name].is_boolean:
                    evidence[var_name] = "True"
                else:
                    raise ValueError(f"Non-boolean variable '{var_name}' must specify a value in evidence")
                    
        return evidence
        
    def _parse_query_variables(self, query_part: str) -> Tuple[List[str], Dict[str, str]]:
        """
    ...
    def _execute_inference(self, query_vars_names: List[str], 
                            evidence: Dict[str, str], 
                            query_evidence: Dict[str, str]):
        """
        ...
        if query_evidence:
            ...
            try:
                assignment_tuple = tuple(query_evidence[var.name] for var in result_factor.variables)
                prob = result_factor.probabilities[assignment_tuple]
                
                from .network_model import Factor
                final_assignment_str = ", ".join([f"{k}={v}" for k, v in query_evidence.items()])
                if evidence:
                    final_assignment_str += " | " + ", ".join([f"{k}={v}" for k, v in evidence.items()])

                return Factor(variables=tuple(), probabilities={tuple(): prob}, name=f"P({final_assignment_str})")

            except (KeyError, StopIteration):
                raise ValueError("Could not find the specified assignment in the result.")
    ...
    def parse_and_execute(self, query_str: str):
        ...
        evidence, query_evidence = self._validate_and_normalize_values(query_vars_names, evidence, query_evidence)
