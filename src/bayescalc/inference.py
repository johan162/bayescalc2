...
class Inference:
    ...
    def _sum_out(self, factor: Factor, var_to_sum_out: Variable) -> Factor:
        ...
        for old_assignment, prob in factor.probabilities.items():
            new_assignment = tuple(old_assignment[i] for i in range(len(old_assignment)) if i != var_index)
            new_factor.probabilities[new_assignment] = new_factor.probabilities.get(new_assignment, 0.0) + prob
            
        return new_factor

    def variable_elimination(self, query_vars: Dict[str, str], evidence: Dict[str, str]) -> Factor:
        """
        ...
        for f in factors:
            if any(v.name in evidence for v in f.variables):
                ...
                for assignment, prob in f.probabilities.items():
                    ...
                    if consistent:
                        new_assignment = []
                        new_vars_temp = []  # Reinitialize for each consistent assignment
        ...
        elimination_order = sorted(
            vars_to_eliminate,
            key=lambda var: sum(1 for f in factors if self.network.variables[var] in f.variables)
        )
        ...
        for var_name in elimination_order:
            ...
            factors_without_var = [f for f in factors if var_to_eliminate not in f.variables]
