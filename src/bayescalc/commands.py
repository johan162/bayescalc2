...
class CommandHandler:
    ...
    def execute(self, command_str: str):
        ...
        if command_str in self.alias_to_command:
            ...
            if not cmd_info['requires_args']:
                ...
                raise ValueError(f"Command '{command_str}' requires arguments. Use: {cmd_info['help']}")

        # Parse command with parentheses
        match = re.match(r"(\w+)\((.*)\)", command_str)
        if not match:
            raise ValueError(f"Invalid command format: {command_str}. Use 'help()' to see available commands.")
        ...
        if command not in self.alias_to_command:
            raise ValueError(f"Unknown command: {command}. Use 'help()' to see available commands.")
        ...
        if not cmd_info['requires_args']:
            if args_str.strip():
                raise ValueError(f"Command '{command}' does not take arguments. Use: {cmd_info['help']}")
            return cmd_info['handler']()
        ...
        if cmd_info['validate_args']:
            ...
            if len(args) != expected_count:
                if expected_count == 1:
                    raise ValueError(f"{cmd_name} requires one argument. Use: {cmd_info['help']}")
                elif expected_count == 2:
                    raise ValueError(f"{cmd_name} requires two arguments. Use: {cmd_info['help']}")
                else:
                    raise ValueError(f"{cmd_name} requires {expected_count} arguments. Use: {cmd_info['help']}")
    ...
    def _handle_conditional_entropy(self, args_str: str) -> float:
        ...
        parts = args_str.split('|')
        if len(parts) != 2 or len(parts[0].split(',')) != 1 or len(parts[1].split(',')) != 1:
    ...
    def marginals(self, n_str: str) -> str:
        ...
        if n > num_vars:
            raise ValueError(f"n={n} exceeds number of variables ({num_vars}) in the network")
    ...
    def condprobs(self, n_str: str, m_str: str) -> str:
        ...
        try:
            ...
            raise ValueError(f"Invalid arguments: n and m must be integers")
        ...
        if n + m > num_vars:
            raise ValueError(f"n+m={n+m} exceeds number of variables ({num_vars}) in the network")
        ...
        for cond_vars in condition_var_combinations:
            for evid_vars in evidence_var_combinations:
                ...
                cond_var_objects = [self.network.variables[var_name] for var_name in cond_vars]
                evid_var_objects = [self.network.variables[var_name] for var_name in evid_vars]
                
                # Get all possible value assignments for condition variables
                cond_value_assignments = list(product(*(var.domain for var in cond_var_objects)))
                
                # Get all possible value assignments for evidence variables
                evid_value_assignments = list(product(*(var.domain for var in evid_var_objects)))
                ...
                for cond_assignment in cond_value_assignments:
                    for evid_assignment in evid_value_assignments:
                        ...
                        try:
                            conditional_dist = self.inference.variable_elimination(list(cond_vars), evidence)
                            
                            # Get probability for this specific assignment of condition variables
                            probability = conditional_dist.probabilities.get(cond_assignment, 0.0)
                            ...
                            prob_str = f"P({', '.join(cond_strs)} | {', '.join(evid_strs)})"
                            prob_value = f"{probability:.6f}"
    ...
    def print_cpt(self, variable_name: str) -> str:
        ...
        if not parents:
            ...
            col_widths = [len(variable.name), len(col_names[1]), 6]  # Set minimum widths
            data_rows = []
        ...
        # Data rows
        for row_data in data_rows:
            row_parts = [f"{str(cell):<{col_widths[i]}}" for i, cell in enumerate(row_data)]
            row = " | ".join(row_parts)
            lines.append(row)
    ...
    def is_cond_independent(self, var1_name: str, var2_name: str, cond_vars: List[str]) -> bool:
        ...
        for cond_values in product(*cond_evidence_domains):
            ...
            p_ab_given_c = self.inference.variable_elimination([var1_name, var2_name], evidence)
    ...
    def print_jpt(self) -> str:
        ...
        ordered_vars = [self.network.variables[var_name] for var_name in self.network.variable_order]
        ...
        for assignment in all_assignments:
            ...
            assignment_dict = {ordered_vars[i].name: assignment[i] for i in range(len(ordered_vars))}
            
            # Reorder the assignment tuple to match the JPT factor's variable order
            jpt_assignment_tuple = tuple(assignment_dict[var_name] for var_name in jpt_var_names)
    ...
    def list_variables(self) -> str:
        ...
        states_width = max(len(", ".join(var.domain)) for var in self.network.variables.values()) + 2
        ...
        for var_name, var_obj in sorted(self.network.variables.items()):
            ...
            lines.append(f"{var_name:<{var_width}} | {var_type:<{type_width}} | {states:<{states_width}}")
...
if __name__ == '__main__':
    ...
    print("\n--- Independence Test ---")
    print(f"Is Rain independent of Sprinkler? {cmd_handler.execute('isindependent(Rain, Sprinkler)')}")

    print("\n--- Conditional Independence Test ---")
    print(f"Is GrassWet independent of Sprinkler given Rain? {cmd_handler.execute('iscondindependent(GrassWet, Sprinkler | Rain)')}")

    print("\n--- Information Theory ---")
    print(f"Entropy of Rain: {cmd_handler.execute('entropy(Rain)'):.4f}")
    print(f"Conditional Entropy of GrassWet given Rain: {cmd_handler.execute('conditional_entropy(GrassWet|Rain)'):.4f}")
    print(f"Mutual Information between Rain and GrassWet: {cmd_handler.execute('mutual_information(Rain, GrassWet)'):.4f}")
