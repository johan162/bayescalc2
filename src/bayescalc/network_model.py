...
class BayesianNetwork:
    ...
    def add_factor(self, variable_name: str, parent_names: List[str], cpt_entries: Dict[Tuple[str, ...], float]):
    ...
    def _validate_and_complete_cpt(self, factor: Factor, cpt_entries: Dict[Tuple[str, ...], float], variable: Variable, parents: List[Variable]):
        ...
        if not parents:
            ...
            if len(cpt_entries) < len(variable.domain):
                ...
                if len(variable.domain) - len(cpt_entries) == 1:
                    missing_val = [v for v in variable.domain if (v,) not in cpt_entries][0]
                    factor.probabilities[(missing_val,)] = 1.0 - total_prob
                else:
                    raise ValueError(f"Ambiguous auto-completion for '{variable.name}'. Please specify more entries.")

            if abs(sum(factor.probabilities.values()) - 1.0) > 1e-6:
                raise ValueError(f"Probabilities for '{variable.name}' do not sum to 1.")
        else:
            ...
            for p_comb in parent_combinations:
                ...
                if len([k for k in cpt_entries if k[1:] == p_comb]) < len(variable.domain):
                    # Auto-complete
                    if len(variable.domain) - len([k for k in cpt_entries if k[1:] == p_comb]) == 1:
                        missing_val = [v for v in variable.domain if (v,) + p_comb not in cpt_entries][0]
                        factor.probabilities[(missing_val,) + p_comb] = 1.0 - total_prob
                    else:
                        raise ValueError(f"Ambiguous auto-completion for '{variable.name}' given {p_comb}. Please specify more entries.")

                if abs(sum(factor.probabilities[(v,)+p_comb] for v in variable.domain) - 1.0) > 1e-6:
                    raise ValueError(f"Probabilities for '{variable.name}' given {p_comb} do not sum to 1.")
